        ...
        cars.append(
            {
                ...
                'date': item.find('div', class_='listing-item__price').get_text(strip=True),
...
def save_doc(items, path):
    with open(path, 'w', newline='') as file:
        ...
        writer.writerow(['title', 'link_car', 'date'])
        for item in items:
            writer.writerow([item['title'], item['link_car'], item['date'], ])
